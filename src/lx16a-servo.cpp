...
bool LX16ABus::write_no_retry(uint8_t cmd, const uint8_t *params, int param_cnt,
    ...
    if (_debug) {
        Serial.printf("SND: ");
        for (int i=0; i<buflen; i++) Serial.printf(" 0x%02x", buf[i]);
        Serial.printf("\n");
    }
    ...
    while (got < buflen && millis()-t0 < tout) {
        if (available()) {
            int ch = read();
            if (_debug) Serial.printf(" 0x%02x", ch);
            if (ch != buf[got]) ok = false;
            got++;
        }
    }
    if (_debug) { if (ok) Serial.printf(" OK\n"); else Serial.printf(" ERR\n"); }
    return ok;
}
...
bool LX16ABus::read_no_retry(uint8_t cmd, uint8_t *params, int param_len,
    ...
    bool ok = write_no_retry(cmd, NULL, 0,MYID);
    if (!ok) return false;
    ...
    while (got < len && millis()-t0 < tout) {
        if (available()) {
            ...
            case 0:
            case 1:
                if (ch != 0x55) { if (_debug) Serial.printf(" ERR (hdr)\n"); return false; }
                break;
            case 2:
                if (ch != MYID && MYID != 0xfe) { if (_debug) Serial.printf(" ERR (id)\n"); return false; }
                break;
            case 3:
                if (ch < 3 || ch > 7) { if (_debug) Serial.printf(" ERR (len)\n"); return false; }
                len = ch+3;
                if (len > param_len+6) { if (_debug) Serial.printf(" ERR (param_len)\n"); return false; }
                break;
            case 4:
                if (ch != cmd) { if (_debug) Serial.printf(" ERR (cmd)\n"); return false; }
                break;
            default:
                if (got == len-1) {
                   if ((uint8_t)ch == (uint8_t)~sum) { if (_debug) Serial.printf(" OK\n"); return true; }
                   else { if (_debug) Serial.printf(" ERR (cksum!=%02x)\n", ~sum); return false; }
                }
                if (got-5 > param_len) { if (_debug) Serial.printf(" ERR (cksum)\n"); return false; }
                params[got-5] = ch;
            }
            if (got > 1) sum += ch;
            got++;
    ...
    if (_debug) Serial.printf(" TIMEOUT\n");
    return false;
}
